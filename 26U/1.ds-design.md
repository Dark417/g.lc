...
- [H] [295. Find Median from Data Stream](#lc-0295)  
  — Maintain running median with two heaps.  
...
Idea: Lazy transfer — push to `in_stack`; on pop/peek, dump `in_stack` into `out_stack` (reversing order) only when `out_stack` is empty.  
Amortized O(1).
...
class Twitter:
    ...
    def getNewsFeed(self, userId):
        heap = []
        followed = self.follows.setdefault(userId, set())
        followed.add(userId)
        for uid in followed:
            tw = self.tweets.get(uid, [])
            if tw:
                i = len(tw) - 1
                t, tid = tw[i]
...
Idea: Map `value -> sorted list of indices` (built in index order, so already sorted); the count is `bisect_right(right) - bisect_left(left)`.
...
class RangeFreqQuery:
    def __init__(self, arr: list[int]):
        self.pos = {}  # value -> [indices]
        for i, x in enumerate(arr):
            self.pos.setdefault(x, []).append(i)

    def query(self, left: int, right: int, value: int) -> int:
        buk = self.pos.get(value, [])
        return bisect_right(buk, right) - bisect_left(buk, left)
# Time: init O(n), query O(log n), Space: O(n)
```
