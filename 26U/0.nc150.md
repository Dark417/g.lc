...
NeetCode 150 in NeetCode topic order.  
Index-first, then detailed solutions.  
Overlaps with Blind 75 reuse copied solution blocks from `26U/blind75.md`.
...
Keep one set per row, column, and box.  
...
Water at a position is bounded by the smaller side.  
Finalize the shorter side first while maintaining left and right maximum walls.
...
Idea: Sliding window; keep `max_count` of any char in the window.  
...
The deque front is always the maximum for the current window.
...
You can place `(` while openings remain.  
You can place `)` only when it will not invalidate the prefix.
...
Store indices whose warmer answer has not been found yet.  
...
Process cars from nearest to target backward.  
A car that would arrive later than the fleet ahead becomes a new fleet.
...
That monotonicity makes the answer binary-searchable.
...
Interpret `nums[i]` as a next pointer.  
The duplicate creates a cycle whose entry point is the answer.
...
Keep items in recency order.  
Every hit moves a key to the end, and overflow evicts the least-recently-used key.
...
For each group, first confirm `k` nodes exist.  
Then reverse that segment and reconnect it before moving forward.
...
The tight lower bound comes from the most frequent task buckets.  
Compare that idle-aware formula with the raw task count.
...
Store each user's tweets with a global time.  
To build the feed, merge the latest tweet streams from followed users.
...
All gates start at distance 0.  
BFS from them simultaneously ensures the first distance written to a room is optimal.
...
Idea: Reverse the flow: start DFS/BFS from ocean borders and move to equal/higher heights.  
Intersection are cells reaching both oceans.
...
Any `O` connected to the border is safe.  
Mark those first, then flip only the remaining `O`s.
...
Idea: A valid tree has exactly `n-1` edges and no cycles.  
...
Reverse-sort adjacency so popping gives the smallest lexical edge next.
...
- **Solution:** [Existing Python solution in Blind 75](0.blind75.md#lc-0269).
...
Idea: Circle constraint means you can't take both first and last.  
Solve two linear cases and take max.
...
Idea: DP over prefix length.  
A digit can stand alone (1..9) and/or pair with previous (10..26).
...
If total sum is odd, answer is impossible.  
Otherwise ask whether `total // 2` is reachable.
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        total = sum(nums)
        # 不可能达到 target，或者无法得到整数 subset sum
        ...
        dp[0] = 1
        for num in nums:
            # 0/1 Knapsack：每个 num 只能用一次，所以倒序
...
class Solution:
    def minDistance(self, word1: str, word2: str) -> int:
        ...
        @cache
        def dfs(i, j):
            # word1 用完了，只能把 word2 剩下的字符全部 insert
            if i == m:
                return n - j
            # word2 用完了，只能把 word1 剩下的字符全部 delete
            if j == n:
                return m - i
...
Track the current reachable range and the farthest next range.  
Each time you exhaust the current range, you commit one jump.
...
# jump from index 0, so 0 don't need jump
class Solution:
...
Any triplet exceeding target in one coordinate can never participate.  
Among valid ones, independently check whether each target coordinate can be matched.
...
If the process does not reach 1, it must loop.  
A seen set detects that cycle quickly.
...
Idea: Bitwise addition: `xor` gives sum without carry; `(a&b)<<1` gives carry.  
