...
- [M] [286. Islands and Treasure](#lc-0286) — Key interview pattern for this problem.
...
class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        for i in range(len(numbers)):
            if target - numbers[i] in dc:
                return [dc[target - numbers[i]], 1 + 1]
            dc[numbers[i]] = i + 1
        return []
...
class Solution:
    def trap(self, height: List[int]) -> int:
        """
        Key idea:
        - Water at each index depends on min(max_left, max_right).
        - If height[l] < height[r], the right side is already a sufficient boundary.
        - So water at l only depends on left_max.
        - Symmetric logic applies to the right side.
        """
        l = 0
        r = len(height) - 1
        left_max = 0
    ...
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for i in range(len(s2)):
            ...
            for j in range(i, len(s2)):
                ...
                if cnt1.get(s2[j], 0) < cnt2.get(s2[j]):
                    break
                if cnt1.get(s2[j], 0) == cnt2.get(s2[j]):
                    cur += 1
                if cur == need:
...
class SolutionHeap:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        - Store (-value, index).
        - Lazy remove expired indices from heap top.
        """
        heap = []
        res = []
        for r, x in enumerate(nums):
            heapq.heappush(heap, (-x, r))
            while heap[0][1] <= r - k:
                heapq.heappop(heap)
            if r >= k - 1:
                res.append(-heap[0][0])
...
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        idx = bisect.bisect_right(col1, target) - 1
        if idx < 0:
            return False
        target_row = matrix[idx]
        ridx = bisect.bisect_left(target_row, target)
        return ridx < len(target_row) and target_row[ridx] == target
...
- comparing to the right is "stable" because it handles both rotated and non-rotated lists automatically.
- comparing to the left is "unstable"
...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        self.m.setdefault(key, SortedDict())[timestamp] = value
...
class Solution:
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # 2. Split and Reverse from slow.next
        cur = slow.next
        slow.next = None # Explicit Cut
        pre = None
        while cur:
            nxt = cur.next
            cur.next = pre
...
class Solution:
    def reorderList(self, head: Optional[ListNode]) -> None:
        ...
        # 3. Weave using head2.next
        head1, head2 = head, pre
        while head2.next: # Stops early to avoid re-linking the tail
            n1, n2 = head1.next, head2.next
            head1.next = head2
            head2.next = n1
            head1, head2 = n1, n2
...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while cur:
            copy = get_copy(cur)
            copy.val = cur.val
            copy.next = get_copy(cur.next)
            copy.random = get_copy(cur.random)
            cur = cur.next
...
class Solution:
    def addTwoNumbers(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        def add(l1, l2, carry):
            ...
            next = add(l1.next if l1 else None, l2.next if l2 else None, carry)
...
```python
# i is tiebreaker: equal val, node
class Solution:
...
def levelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
    ...
    res, q = [], [root]
    while q:
        # Save all values in the current list
...
```python
# calc each level if good
# pass down, if path all good
...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ct = Counter(tasks)
        mf = max(ct.values())
        mfn = sum(1 for f in ct.values() if f == mf)
        return max(len(tasks), (mf - 1) * (n + 1) + mfn)
...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        for i in range(24, -1, -1):
            idle -= min(ct[i], maxF - 1)
        return len(tasks) + max(0, idle)

class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ct = Counter(tasks)
        h = [-cnt for cnt in ct.values()]
        heapq.heapify(h)
        time = 0
        q = deque()
        while h or q:
...
class Twitter:
    ...
    def getNewsFeed(self, userId):
        ...
        for followee in self.following[userId]:
            posts = self.tweets.get(followee, [])
            if posts:
                idx = len(posts) - 1
...
def subsets(nums):
    ...
    def backtrack(start, path):
        # Every state of 'path' is a valid subset
        res.append(path[:])
...
# don't have to include path in arg
class Solution:
...
def subsets(nums):
    res = [[]]
    for n in nums:
        # Create new subsets by adding 'n' to existing ones
...
- split string into parts, each is palidrome
-
...
- Fill each empty land cell with its distance to the nearest treasure using the same multi-source BFS pattern as Walls and Gates.
...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def dfs(r, c, visit, preH):
            if ((r, c ) in visit or r < 0 or c < 0 or r == ROWS or c == COLS or heights[r][c] < preH):
                return
            visit.add((r, c))
...
class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        @cache # This one line makes backtracking efficient!
        def dfs(rem):
            if rem == 0: return 0
...
class Solution:
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        for i in range(1, n + 1):
            for w in wordDict:
                word_len = len(w)
                if i >= word_len and dp[i - word_len] and s[i - word_len : i] == w:
                    dp[i] = True
                    break # Found a way to reach index i, move to i+1
        return dp[n]
...
def minMeetingRooms(intervals):
    ...
    # Sort intervals by their .start attribute
    intervals.sort(key=lambda x: x.start)

    # Heap stores the .end times of active meetings
    free_rooms = []
    ...
    for i in range(1, len(intervals)):
        # If the room that finishes earliest is free by the next .start
...
class DetectSquares:
    ...
    def add(self, point):
        x, y = point
        self.points[(x, y)] += 1
        self.rows.setdefault(y, Counter())[x] += 1

    def count(self, point):
        x, y = point
        total = 0

        for col, freq in self.rows.get(y, {}).items():
            if col == x:
                continue
            side = col - x
