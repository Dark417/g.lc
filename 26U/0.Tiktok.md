...
- [M] [90. 子集 II](#lc-0090) \
  Sort first and skip duplicate siblings during DFS so each subset appears exactly once. \
  `Backtracking` `DFS` `Dedup`  

- [M] [216. 组合总和 III](#lc-0216) \
...
- [M] [628. 三个数的最大乘积](#lc-0628) \
  排序后比较最大三数与两个最小一最大组合的乘积。 \
  `Array` `Math`  

- [M] [238. 除了自身以外数组的乘积](#lc-0238) \
...
- [M] [740. 删除并获得点数](#lc-0740) \
  将值映射到点数，转化为打家劫舍求最大值。 \
  `Array` `Dynamic Programming` `Hash Table`  

- [M] [2611. 老鼠和奶酪](#lc-2611) \
...
- [1756. Design Most Recently Used Queue](#lc-1756)
- [297. 二叉树的序列化与反序列化](#lc-0297)
- [428. 序列化和反序列化 N 叉树](#lc-0428)
- [449. 序列化和反序列化二叉搜索树](#lc-0449)
- [57. 插入区间](#lc-0057)
- [435. 无重叠区间](#lc-0435)
- [252. 会议室](#lc-0252)
- [253. 会议室 II](#lc-0253)
...
class Solution:
    def groupStrings(self, strings: List[str]) -> List[List[str]]:
        ...
        for s in strings:
            ...
            dc.setdefault(tuple(key), []).append(s)
...
class LFUCache:
    ...
    def get_dummy(self, freq: int) -> Node:
        if freq not in self.freq_to_dummy:
            dummy = Node()  # 哨兵节点
            dummy.prev = dummy
            dummy.next = dummy
            self.freq_to_dummy[freq] = dummy
        return self.freq_to_dummy[freq]
    ...
    def get_node(self, key: int) -> Optional[Node]:
        ...
        if dummy.prev == dummy:  # 抽出来后，这摞书是空的
            ...
            if self.min_freq == node.freq:  # 这摞书是最左边的
                self.min_freq += 1
        node.freq += 1  # 看书次数 +1
        self.push_front(self.get_dummy(node.freq), node)  # 放在右边这摞书的最上面
        return node
    ...
    def put(self, key: int, value: int) -> None:
        ...
        self.push_front(self.get_dummy(1), node)  # 放在「看过 1 次」的最上面
        self.min_freq = 1
...
class LFUCache:
    def __init__(self, capacity: int):
        ...
        self.freq_to_dict = {}  # freq -> OrderedDict（只保存 key）
    ...
    def get_list(self, key: int) -> Optional[List[int]]:
        ...
        if not self.freq_to_dict[self.min_freq]:  # 最左边那摞书是空的
            self.min_freq += 1
        lst[1] += 1  # 看书次数 +1
        next_group = self.freq_to_dict.setdefault(freq + 1, OrderedDict())
        next_group[key] = None
        next_group.move_to_end(key, last=False)  # 放在右边这摞书的最上面
        return lst
    ...
    def put(self, key: int, value: int) -> None:
        ...
        first_group = self.freq_to_dict.setdefault(1, OrderedDict())
        first_group[key] = None
        first_group.move_to_end(key, last=False)  # 放在「看过 1 次」的最上面
        self.min_freq = 1
...
class LFUCache:
    ...
    def get_freq_list(self, freq):
        return self.freqMap.setdefault(freq, create_linked_list())

    def delete(self, node):
        if node.pre:
            node.pre.nex = node.nex
            node.nex.pre = node.pre
            if node.pre is self.get_freq_list(node.freq)[0] and node.nex is self.get_freq_list(node.freq)[-1]:
    ...
    def increase(self, node):
        node.freq += 1
        self.delete(node)
        self.get_freq_list(node.freq)[-1].pre.insert(node)
        if node.freq == 1:
            self.minFreq = 1
        elif self.minFreq == node.freq - 1:
            head, tail = self.get_freq_list(node.freq - 1)
            if head.nex is tail:
    ...
    def put(self, key: int, value: int) -> None:
        if self.capacity != 0:
            ...
            if self.size > self.capacity:
                self.size -= 1
                deleted = self.delete(self.get_freq_list(self.minFreq)[0].nex)
