...
| 13  | [大文件上传容错 (chunk / resume) →sd13](sd13.file-upload.md) | 1p3a | HLD component | P0 |
| 14  | Chat app (WhatsApp/Slack, 1B / 10M DAU)   | 1p3a               | HLD               | P0              |
| 15  | Design Uber (matching, location, trip)    | 1p3a ×2            | HLD               | P0 (排后: 消费级) |
| 16  | [Distributed job scheduler / orchestrator →sd16](sd16.job-scheduler.md) | ★★★ | HLD + concurrency | 🎯 P0 |
| 17  | [Distributed KV store →sd17](sd17.kv-store.md) | Alex Xu + ◇   | HLD               | P0              |
| 18  | [Distributed lock manager (lease, fencing) →sd18](sd18.distributed-lock.md) | ◇ | LLD/HLD | 🎯 P1 |
| 19  | [Certificate management service →sd19](sd19.certificate-management.md) | team domain | HLD | 🎯 P1 |
| 20  | [Control-plane work-request API →sd20](sd20.work-request-api.md) | JD | HLD | 🎯 P1 |
| 21  | [Kafka-like distributed log →sd21](sd21.kafka-log.md) | ◇ | HLD | 🎯 P1 |
| 22  | [Thread-safe DB connection pool →sd22](sd22.connection-pool.md) | ◇ | LLD | 🎯 P1 |
| 23  | [Distributed file / object storage →sd23](sd23.object-storage.md) | ◇ | HLD | 🎯 P1 |
...
- Full files now cover the distributed/platform set (2026-08-31): sd16–sd02 + **sd06 LB · sd05 ingestion (metrics + healthcare) · sd08 edge health · sd04 logging FS · sd21 Kafka log · sd22 connection pool · sd23 object storage · sd13 file upload**
...
1. Distributed job scheduler — `sd16.job-scheduler.md`
...
3. Certificate management service — `sd19.certificate-management.md`
   → CP: strong consistency, HSM-signed, audited · DP: LB caches cert, survives CP outage
4. Distributed lock manager — `sd18.distributed-lock.md`
   → lease with TTL; fencing token monotonic; ZooKeeper/etcd vs Redis Redlock
5. Control-plane work-request API — `sd20.work-request-api.md`
   → 202 + work-request id; state machine; idempotent retries; per-tenant quotas; audit
6. Distributed KV store — `sd17.kv-store.md`
   → consistent hashing; leader vs leaderless replication; quorum R+W>N; anti-entropy
7. URL shortener — quick — `sd02.url-shortener.md`
8. First-hand / infra set (2026-08-31): `sd06` LB · `sd05` ingestion pipeline · `sd08` edge health · `sd04` logging FS · `sd21` Kafka-like log · `sd22` connection pool · `sd23` object storage · `sd13` file upload
...
- DB connection pool → `sd22.connection-pool.md`
...
- Assessment
  ...
  → plausible for platform team: lock manager, connection pool, log, object storage — all now have full files (sd18, sd22, sd21, sd23)
...
- HLD: client library → consistent-hash ring of cache nodes (sd17 §10.1 ring, no replication or N=2 async) → backing store on miss
- Data: values with TTL + version; no durability promises; `mc:{tenant}:{key}` namespacing
- Reuse: ring from sd17; stampede/`getOrLoad` + two-tier from sd03 §10.4; invalidation via pub/sub
- Defeater: "replicate cache nodes for durability" — a cache that must not lose data is a KV store; call it what it is (sd17)
...
- HLD: upload (chunked, sd13 的模式) → 转码集群 (多分辨率 HLS/DASH 段) → object storage (sd23) → CDN; 播放走 manifest + 自适应码率; 元数据/推荐单独服务
- Reuse: Alex Xu vol 1 YouTube 章; upload 容错 = sd13
...
- Reuse: gateway/quota = sd01; 租户隔离/审计 = sd19-07 词汇; edge = CDN 思路
...
- HLD: 行为事件 → Kafka (sd21) → 特征存储; 离线训练 (协同过滤/双塔) 产出 embedding + 候选索引; 在线: 召回 (ANN/规则) → 排序 (轻模型) → 过滤去重; 冷启动用热门/类目
...
- Reuse: Alex Xu vol 1 chat 章; presence = sd08 心跳模型; 幂等 = sd20 §9.1
...
- 60 s answer: prepare (all vote, persist redo) → commit; coordinator log is truth; blocks on coordinator crash between phases — participants hold locks → prefer sagas/outbox (sd20 §9.2, sd16 §10.2) for cross-service; 2PC only intra-trust-domain (XA, spanner-style with Paxos coordinator)
...
- HLD: browse (cached, stale-ok) → hold seat (TTL lease, sd18 §8 row-lease pattern: `UPDATE seats SET state='HELD', hold_until=... WHERE seat_id=? AND state='FREE'`) → pay → confirm (state machine HELD→BOOKED, fence = version column)
...
- HLD: cart in KV (sd17; LWW-with-merge: union items, max qty — say the merge rule) → checkout = saga: reserve inventory → charge → confirm, compensations in reverse (sd20 §9.2)
- Data: `carts(user_id) → items jsonb` in KV; `inventory(sku, available, version)` optimistic-locked in SQL; `orders` state machine
- Reuse: idempotent charge via Idempotency-Key (sd20 §9.1); JPMC anchor: Aurora canonical + Mongo read model (CQRS) — your own story
...
- vs LSM (sd17 §9): B-tree = read-optimized in-place; LSM = write-optimized append; that contrast is usually the actual question
