...
> Bank rows 1–15 = **first-hand 1p3a reports (P0)** — details in `../1.coding/0.1p3a.md`; rows 16–24 = distributed/platform core; rows 25+ = rest.
...
Rows 1–15 = first-hand (P0, 前置; Uber 排到块尾); rows 16–24 = distributed/platform core (全部有 full file 或 60-s 答案); rows 25+ = 其余。

| #   | Prompt                                    | Evidence           | Type              | Priority        |
| --- | ----------------------------------------- | ------------------ | ----------------- | --------------- |
| 1   | [Rate limiter (distributed) →sd02](sd02.rate-limiter.md) | 1p3a ×2 + recruiter + ◇ | HLD + algo | 🎯 P0 |
| 2   | [Cache system: thread-safe LRU (LLD) → distributed cache →sd03](sd03.thread-safe-cache.md) | 1p3a ×2 + ★★★ + ◇ | LLD + HLD | 🎯 P0 |
| 3   | [URL shortener / bitly →sd08](sd08.url-shortener.md) | 1p3a + ★★★ | HLD | P0 (easy) |
| 4   | [Logging system / log file system →sd12](sd12.logging-file-system.md) | 1p3a + ★ + ◇ | LLD | P0 |
| 5   | [Healthcare data ingestion pipeline →sd10 §12](sd10.ingestion-pipeline.md) | 1p3a | HLD | P0 |
| 6   | [Software load balancer (L4 / L7) →sd09](sd09.load-balancer.md) | 1p3a | HLD | 🎯 P0 |
| 7   | [Internal metric ingestion service →sd10](sd10.ingestion-pipeline.md) | 1p3a | HLD | 🎯 P0 |
| 8   | [Edge-device health monitoring @ millions →sd11](sd11.edge-health-monitoring.md) | 1p3a | HLD | 🎯 P0 |
| 9   | Scale a monolithic service                | 1p3a               | HLD discussion    | P0              |
| 10  | Video streaming (YouTube/Netflix, global) | 1p3a               | HLD               | P0              |
| 11  | Remote Browser Isolation (enterprise)     | 1p3a               | HLD               | P0              |
| 12  | E-commerce recommendation system          | 1p3a               | HLD               | P0              |
| 13  | [大文件上传容错 (chunk / resume) →sd16](sd16.file-upload.md) | 1p3a | HLD component | P0 |
| 14  | Chat app (WhatsApp/Slack, 1B / 10M DAU)   | 1p3a               | HLD               | P0              |
| 15  | Design Uber (matching, location, trip)    | 1p3a ×2            | HLD               | P0 (排后: 消费级) |
...
| 18  | [Distributed lock manager (lease, fencing) →sd05](sd05.distributed-lock.md) | ◇ | LLD/HLD | 🎯 P1 |
| 19  | [Certificate management service →sd06](sd06.certificate-management.md) | team domain | HLD | 🎯 P1 |
| 20  | [Control-plane work-request API →sd07](sd07.work-request-api.md) | JD | HLD | 🎯 P1 |
| 21  | [Kafka-like distributed log →sd13](sd13.kafka-log.md) | ◇ | HLD | 🎯 P1 |
| 22  | [Thread-safe DB connection pool →sd14](sd14.connection-pool.md) | ◇ | LLD | 🎯 P1 |
| 23  | [Distributed file / object storage →sd15](sd15.object-storage.md) | ◇ | HLD | 🎯 P1 |
| 24  | Distributed transaction coordinator (2PC) | ◇ (DB group)       | HLD               | discussion only |
...
2. Cache system — 一轮: 设计 LRU 并解释数据结构选型; 另一轮: design a cache system; LLD → 分布式两段都要能接
3. URL shortener — 轮末最后 10 分钟才出题, 无大问题
...
- Full files now cover the distributed/platform set (2026-08-31): sd01–sd08 + **sd09 LB · sd10 ingestion (metrics + healthcare) · sd11 edge health · sd12 logging FS · sd13 Kafka log · sd14 connection pool · sd15 object storage · sd16 file upload**
...
8. First-hand / infra set (2026-08-31): `sd09` LB · `sd10` ingestion pipeline · `sd11` edge health · `sd12` logging FS · `sd13` Kafka-like log · `sd14` connection pool · `sd15` object storage · `sd16` file upload
...
- DB connection pool → `sd14.connection-pool.md`
...
- Logging system — append, rotate, query by time range → `sd12.logging-file-system.md`
...
- Assessment
  ...
  → plausible for platform team: lock manager, connection pool, log, object storage — all now have full files (sd05, sd14, sd13, sd15)
...
### #2 Cache system — distributed-cache half (P0; LRU half = sd03)
...
### #10 Video streaming global (P0)

- Crux: 上传/转码 (异步流水线) 与播放 (CDN 边缘) 完全分离; 全球分发 = CDN + 区域源站
- HLD: upload (chunked, sd16 的模式) → 转码集群 (多分辨率 HLS/DASH 段) → object storage (sd15) → CDN; 播放走 manifest + 自适应码率; 元数据/推荐单独服务
- Reuse: Alex Xu vol 1 YouTube 章; upload 容错 = sd16
- Defeater: "源站直接服务视频" — 带宽成本与延迟; CDN 命中率是第一指标
- 报告注: 题面从 URL (schemaName, host, port) 起步 — 先把解析/路由讲清再进入流媒体

### #11 Remote Browser Isolation (P0)
...
### #12 E-commerce recommendation system (P0)

- Crux: 离线/近线/在线三层 — 浏览历史进流, 模型离线训, 在线只做召回+排序
- HLD: 行为事件 → Kafka (sd13) → 特征存储; 离线训练 (协同过滤/双塔) 产出 embedding + 候选索引; 在线: 召回 (ANN/规则) → 排序 (轻模型) → 过滤去重; 冷启动用热门/类目
- Defeater: "在线实时训练" — 延迟与稳定性; 近线更新 embedding 已足够
- 报告注: 基于浏览历史 — 讲清事件采集 → 特征 → 召回排序链路即可

### #14 Chat app 1B / 10M DAU (P0)
...
- Reuse: Alex Xu vol 1 chat 章; presence = sd11 心跳模型; 幂等 = sd07 §9.1
...
- Reuse: heartbeat + presence = sd11 push 模型; surge/quota = sd02 thinking
...
### #24 Distributed transaction coordinator / 2PC (discussion only)
