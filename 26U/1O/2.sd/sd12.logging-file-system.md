# sd12. Logging File System (LLD → HLD)
...
- Sealed segments upload to object storage (sd15); a central indexer registers them → cross-node query = same binary-search over a global segment_meta
- Tail-shipping (live): the drain loop tees batches to Kafka (sd10 pipeline) for real-time search/alerting
...
- **L5:** this is Kafka's storage layer in miniature — segments + index + retention (sd13 §storage)
