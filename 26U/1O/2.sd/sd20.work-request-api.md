...
> Cross-ref: `sd16` (the executor behind it), `sd19` (its main client), `../4.oci-architecture.md` §2 (CP vs DP).
...
- NFR: submit p99 < 150 ms (validate + persist only) · status read 99.99% (it's what everyone polls) · state strongly consistent · audit every transition · per-tenant quotas + fairness (one tenant's storm can't starve others — `sd16 §10.3`)
...
[async]     Relay ─► queue ─► Workers (sd16 machinery: lease + fence) ─► call downstream / drive steps
...
- Walkthrough — create certificate:
  ...
  2. Worker leases wr → `IN_PROGRESS` → HSM sign (sd19) → cert `ACTIVE`, wr `SUCCEEDED` (percent 100)
...
CREATE TABLE work_requests (
  ...
  fence        bigint NOT NULL DEFAULT 0,  -- sd16 lease/fence for the executing worker
...
-- transitions audited to audit_log (append-only, as sd19)
```
...
def submit(db, tenant, idem_key, operation, payload):
    ...
    with db.txn() as tx:
        ...
        tx.run("INSERT INTO outbox (wr_id) VALUES (%s)", (wr_id,))         # sd16 §10.2
        return wr_id, True
...
- Reaper: `IN_PROGRESS` with expired lease → re-queue (fence bumps, sd16); `ACCEPTED` older than SLA → alarm (relay stuck)
...
- Quotas: in-flight wr per tenant (e.g., 100) → 429 + `Retry-After`; protects the executor fleet (fairness: sd16 §10.3)
...
- _Exactly-once?_ → "At-least-once execution + idempotent steps + unique effect keys = exactly-once *effect*. Same argument as sd16; the wr row is the dedupe anchor."
...
| Orchestration | own saga on sd16 machinery    | Temporal/SFN         | org already runs one                |
