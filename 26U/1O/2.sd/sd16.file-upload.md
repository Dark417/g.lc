# sd16. Fault-Tolerant Large File Upload (chunk / resume)

> First-hand 1p3a P0 (bank #13): "大文件上传如何增加容错" — asked as a single design question. Client-side mirror of sd15's write path; S3 multipart is the reference API.
...
- Where do bytes land? → object storage (sd15); this design is the protocol + client
...
staging in object store: {upload_id}/{part_no}
  ...
  ▼
verify etags + assemble/commit manifest (sd15 §6)  → object COMMITTED
...
- Complete: verify client's part manifest matches stored (count, etags, contiguous numbering, sizes) → verify assembled hash = declared sha256 → sd15 manifest commit → parts become the object's chunks (no copy needed if part size = chunk size!)
...
- ❓ Browser client? → same protocol; presigned per-part URLs so bytes skip your API tier (sd15 §5 pattern)
