...
- [H] [LCR 143. Substructure Determination](#lc-lcr-413)\
...
`Tree` `Depth-First Search` `Dynamic Programming`  
Description: Unrooted tree with prices.  
For each node as root, cost = max path sum − min path sum (min is always just the root itself).  
Return the max cost.

##### Approach 1: Tree diameter variant

Idea: For any path, answer = path_sum − min_endpoint_price.  
Track two chains per node: one including the far-end leaf price, one excluding it.  
Combine like tree diameter.
...
Idea: Root is the last element of postorder.  
...
Idea: DFS, visiting right subtree first.  
Record node at each new depth.
...
<a id="lc-lcr-413"></a>
#### LCR 143. [Substructure Determination](https://leetcode.cn/problems/shu-de-zi-jie-gou-lcof/) [H]  
`Tree` `Depth-First Search` `Binary Tree`  
...
Idea: Preorder traversal with `N` for null nodes.  
...
Idea: BFS serialization using `N` for nulls.  
Deserialize with a queue pairing parents with children.
...
Idea: Post-order DFS.  
Each node returns a state: 0 = not covered, 1 = has camera, 2 = covered.  
Place camera greedily at parent of uncovered child.
...
Idea: dp[node] = (no_cam_covered, has_cam, not_covered).  
Minimise cameras at each subtree.
...
Idea: DFS collecting (col, row, val) triples.  
...
Idea: Parse depth (dash count) and value.  
Use a stack to track the path from root to current node.  
Pop until stack size matches depth, then attach as left or right child.
...
Idea: Precompute `pa[i][x]` = 2^i-th ancestor of x.  
To find k-th ancestor, decompose k into binary and jump.
...
Idea: Represent char parity as bitmask.  
Path u→v = mask[u] ⊕ mask[v].  
Palindrome iff at most 1 bit set.  
Count matching pairs with a hash map.
...
class Solution:
    def countPalindromePaths(self, parent: List[int], s: str) -> int:
        ...
        while st:
            ...
            for v, w in g[u]:
                mask[v] = mask[u] ^ w
                ans += cnt.get(mask[v], 0)
                for k in range(26):
                    ans += cnt.get(mask[v] ^ (1 << k), 0)
                cnt[mask[v]] = cnt.get(mask[v], 0) + 1
                st.append(v)
        return ans
...
Idea: DFS returns subtree sum.  
Whenever a subtree sum is divisible by k, cut it off (return 0) and increment the count.
