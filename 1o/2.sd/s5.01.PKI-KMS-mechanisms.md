...
> JD tie-in ([req packet](../0.req/0.req.md#req-a3)): PKI team — certificate lifecycle, key management, trust services, multi-tenant, backward compatibility. This file is the **vocabulary + mechanism** layer; the interview-format design of the same domain is [s2.01](s2.01.Multi-tenant-certificate-authority.md), the OCI product mapping is `../4.1o-arch.md` §3/§5.
> Tags: `[hot path]` · `[async]` · `[control plane]`. ❓ = likely follow-up + the answer to give. 🗣 = say verbatim.

## Index

- [Part A — What a PKI is](#s501-a)
  - [A.1 The problem PKI solves](#s501-a1)
  - [A.2 The objects: key pair, certificate, CA, chain, trust store, revocation](#s501-a2)
  - [A.3 Public key vs. certificate — the relation](#s501-a3)
  - [A.4 What the Oracle PKI doc actually describes](#s501-a4)
  - [A.5 Anatomy of an X.509 v3 certificate + how a verifier walks it](#s501-a5)
  - [A.6 Revocation options compared (CRL / OCSP / stapling / short-lived)](#s501-a6)
  - [A.7 Algorithms, file formats, and the openssl/keytool one-liners](#s501-a7)
- [Part B — KMS vs. PKI vs. Secrets (AWS KMS / OCI Vault)](#s501-b)
  - [B.1 Taxonomy: four different services people lump together](#s501-b1)
  - [B.2 AWS KMS features](#s501-b2)
  - [B.3 Rotation — what actually rotates](#s501-b3)
  - [B.4 Access: role → authenticated → authorized → key used](#s501-b4)
  - [B.5 OCI equivalents (Vault, Certificates, IAM)](#s501-b5)
  - [B.6 KMS extras: key states, grants, DEK caching, `Sign` for a CA](#s501-b6)
- [Part C — Design doc: internal Certificate Authority service](#s501-c)
  - [C.0 One-page design doc](#s501-c0)
  - [C.1 Requirements](#s501-c1)
  - [C.2 High-level design](#s501-c2)
  - [C.3 APIs](#s501-c3)
  - [C.4 Data model](#s501-c4)
  - [C.5 Core flows (numbered sequences)](#s501-c5)
  - [C.6 Failure modes](#s501-c6)
  - [C.7 MVP ladder + what we evolve next](#s501-c7)
  - [C.8 Key function code: `issue(csr)`](#s501-c8)
  - [C.9 Build vs buy: managed CAs, Vault PKI, step-ca, SPIRE, cert-manager](#s501-c9)
  - [C.10 ❓ Pop-up questions (IC3 baseline + IC4 stretch)](#s501-c10)
- [🗣 One-liners for the interview](#s501-oneliners)
- [References](#s501-refs)
---
...
| --- | --- | --- | --- | --- | --- |
| Mass expiry outage | CA down longer than the leaf TTL, or a renewal-agent bug | Every service whose cert expires inside the window | `certs_expiring_within_1h` gauge; renewal error rate | Already-issued certs keep working until their own `notAfter`; TLS fails service by service as each expires | Renew at 50% of TTL (12 h for a 24 h cert) so the CA can be down 12 h before impact; alert at 70%; restore the CA and let agents catch up |
| KMS throttling | Deploy storm renews a whole fleet at once | New issuance fails; running services unaffected | 429 rate from KMS | Issue API returns 429 with `Retry-After`; renewals back off and retry | Idempotent retries with jitter; per-environment keys; raise the quota; add renewal jitter to the client library |
| Intermediate key compromise | Issuer host or role stolen | Every certificate signed since the compromise | Anomalous issuance in the audit log — unknown SANs, off-hours volume | Issuance halted for that intermediate; the fleet runs on already-issued certs | Revoke the intermediate via the root CRL, rotate per C.5; short leaf TTL bounds exposure to ≤ 24 h |
| Root key compromise | KMS account compromise | Everything that chains to the root | Any root `Sign` event outside a ceremony window | No safe degraded mode; the whole trust anchor is suspect | Break-glass: new root, cross-sign, redistribute trust stores; root policy allows `Sign` only from an MFA-approved role and alarms on every use |
| Stale CRL or OCSP down | Publisher failure | Verifiers soft-fail and accept revoked certs, or hard-fail and reject everything, depending on their config | `nextUpdate − now` gauge on the published CRL | Soft-fail verifiers keep serving with stale revocation state; hard-fail verifiers reject all | Publish at an interval well under the CRL validity; serve from a CDN; prefer short-lived certs so revocation carries less weight |
| Orphan cert — signed but not recorded | DB failure between the sign and insert steps | One certificate, for at most its TTL | Outbox reconciliation finds a serial absent from the inventory | The cert works but cannot be revoked by serial | Reconciler inserts the serial as `revoked`; short TTL bounds the window; never return a cert whose insert failed |
| Clock skew | Node NTP drift | "Certificate not yet valid" at startup on the skewed node | `notBefore` rejections in client logs | That node's TLS handshakes fail until its clock or the cert catches up | Backdate `notBefore` by 5 minutes; monitor NTP offset |
| Policy misconfiguration allows a wildcard SAN | Bad policy pull request merged | Any service can request a cert impersonating any other | Policy diff check in CI; issuance audit review | Issuance continues, so the exposure is silent until audited | Policy lives in Git under review; CI runs a fixture suite of allowed and denied CSRs; per-tenant Name Constraints make the bug unexploitable |
| Audit sink unavailable | Log pipeline outage | Issuance is unattributed for the outage window | Audit write error rate | Issuance continues; records buffer locally | Never block issuance on the audit sink; buffer and replay, alarm on buffer depth |
...
### C.10 ❓ Pop-up questions — IC3 baseline + IC4 stretch

- ❓ **Why does the CA never generate the private key for the requester?**
  → **IC3 baseline:** CSR model = proof of possession; the key exists only where it is used, so a CA breach leaks nothing usable. Key-out (PKCS#12) only for appliances that cannot generate keys, returned once and never stored.
  → **IC4 stretch:** the CSR signature also stops a "confused deputy" issuing for a public key the caller found elsewhere — validate it before policy, so denials do not leak which names exist.
- ❓ **A Java client fails with "unable to find valid certification path" but Chrome opens the same endpoint. What happened?**
  → **IC3 baseline:** the server sends only the leaf; Chrome fetched the intermediate via AIA, Java does not. Fix the chain file (leaf + intermediates), verify with `openssl s_client -showcerts`.
  → **IC4 stretch:** second candidate is AKI/SKI mismatch during an intermediate overlap; third is a JVM whose `cacerts` never got the new root — which is why the trust-bundle agent reports coverage.
- ❓ **How do you make revocation actually work?**
  → **IC3 baseline:** short-lived leaves so revocation ≈ stop renewing; CRL on a CDN as backstop with `nextUpdate` alarms; OCSP stapling only for public endpoints.
  → **IC4 stretch:** name the soft-fail dilemma (A.6) and say which verifiers you control; for the ones you do not, the TTL is your revocation SLA.
- ❓ **Rotate the intermediate with zero outage.**
  → **IC3 baseline:** new key + cert, `overlap` state, distribute the bundle first, switch the signer only when coverage ≥ threshold, keep the old key for CRL signing until its last leaf expires.
  → **IC4 stretch:** the same shape for the root uses cross-signing (C.5); the failure is always a verifier you did not know existed — build the inventory of verifiers before the rotation, not during.
- ❓ **Where is the consistency boundary in issuance, and what is the orphan-cert problem?**
  → **IC3 baseline:** the Postgres insert is the commit; a signed-but-unrecorded cert cannot be revoked by serial. Order: sign → insert → return; on insert failure, do not return the cert, outbox the serial for the reconciler.
  → **IC4 stretch:** it is unavoidable without a distributed transaction across KMS and DB; the accepted risk is bounded by TTL and by the reconciler inserting the orphan as `revoked`. Say the bound out loud.
- ❓ **KMS starts throttling during a deploy storm. What do you do in the next 10 minutes, and next quarter?**
  → **IC3 baseline:** now — retries with jitter are already idempotent, so nothing is duplicated; shed by returning 429 with `Retry-After`; per-env keys spread the quota. Quarter — renewal jitter in the client library, raise the quota, longer TTL for the biggest fleets.
  → **IC4 stretch:** move KMS off the hot path: per-node or per-cluster delegated issuers with short-lived, name-constrained sub-CAs (what SPIRE and Istio do) — KMS then signs one sub-CA per node per day instead of one leaf per pod.
- ❓ **Multi-tenant: how do you stop tenant A from obtaining a cert for tenant B's name?**
  → **IC3 baseline:** identity carries the tenant; policy maps identity → allowed SANs; every issue is audited with the identity.
  → **IC4 stretch:** defense in depth — per-tenant intermediate with Name Constraints, so even a policy bug cannot produce a cross-tenant cert; compartment-scoped authz on the API; anomaly detection on issuance patterns.
- ❓ **Backward compatibility: an Oracle DB client only trusts RSA and reads a wallet. How do you serve it without weakening the rest?**
  → **IC3 baseline:** an `rsa-legacy` profile with RSA-2048, longer TTL, a separate RSA chain, packaged as PKCS#12 for the wallet; CRL published where that client will actually fetch it.
  → **IC4 stretch:** keep the legacy chain on its own intermediate so its longer TTLs and weaker revocation never widen the blast radius of the ECDSA fleet; put a sunset date on the profile and count its consumers monthly.

---

<a id="s501-oneliners"></a>
...
- Repository context
  ...
  - Interview-format design of this same domain — [s2.01](s2.01.Multi-tenant-certificate-authority.md) through [s2.06](s2.06.Regional-PKI-disaster-recovery.md); this file supplies their vocabulary and mechanism layer.
  - Role and requirement source — [req packet](../0.req/0.req.md#req-a3); OCI product mapping — [4.1o-arch.md](../4.1o-arch.md).
  - API and schema companion — [2.rest102.md](../../1sde/1.backend/2.rest102.md).
